...
public class ConsoleChatClient implements Runnable {
	...
	public void service(String serverIp) {
		...
		try{
			...
			//상대방이 보내온 메세지를 받는 쓰레드 < - >  메인 쓰레드 : 내가 보내는 메세지 쓰레드
			
				// === 클라이언트에서 서버쪽으로 메시지를 보내는 작업
				do {
					if(socket.isClosed())  // 소켓이 닫힌 상태이라면(상대방의 서버가 꺼져버린경우)
						//break;
						System.exit(0);//시스템을 꺼버린다.
					else { // 소켓이 열려진 상태이라면
				...
				System.out.println(">>> 채팅클라이언트가 종료합니다. <<<");	//System.out.println은 나에게 글자가 보여짐.
				System.exit(0);
	...
	public void run() {
		...
		try {
				instream = socket.getInputStream();//클라이언트 소켓
